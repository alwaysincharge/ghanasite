...
if (isset($_POST['submit']) )  {
    ...
    if (empty($_FILES['flag']['name']))  {
        ...
        $query1 = "SELECT * FROM mainpage WHERE id = 1 LIMIT 1";
        $result1 = mysqli_query($connection, $query1);
        
        while ($noimg = mysqli_fetch_assoc($result1)) { 
        
        
         $filetmp = $_FILES['flag']['tmp_name'];
    
        $filename = $noimg['img_name'];
    
        $filetype = $noimg['img_type'];
    
        $filepath = $noimg['img_path'];;    
    ...
    if (empty(trim($_POST["main_name"]))  ||  empty(trim($_POST["content"])) ) {
  ...
  All fields of the form have to be filled. <br><br> Images have to be in popular formats.
         ...
         $query = "UPDATE mainpage SET img_name = '$filename', img_path = '$filepath', img_type = '$filetype', main_name = '$main_name', content = '$content' WHERE id = 1";
         
    
    
    $editupload = mysqli_query($connection, $query);
...
<body>
    ...
    <div class="col-md-8">
        ...
        <form method="post" action="edit_mainpage.php" enctype="multipart/form-data" style="max-width: 500px; display: table; margin: 0 auto; padding-left: 30px; padding-right: 30px;">
            
            <p class="form-heading">Edit Main Page.</p>
        ...
        while ($para = mysqli_fetch_assoc($pararesult)) { echo $para['content']; }
         ?></textarea> <br><br>
    ...
    <span class="fileupload-exists" style=" font-family: georgia; font-size: 15px;">Change</span><input type="file" name="flag" placeholder="Upload the Ghana flag" value="<?php 
       $query = "SELECT * FROM mainpage WHERE id = 1 LIMIT 1";  
    ...
    <div class="col-md-4 center">
        <h3 style="font-family: 'Josefin Slab', serif;">Current flag image.</h3>
